...
import { CHUNK_OVERLAP, CHUNK_SIZE, INDEX_ROOT, PROJECT_ROOT } from "./config.mjs";
 ...
 */
export async function indexFile(filePath, col) {
...
async function walk(dirPath, col) {
    for (const entry of await fs.promises.readdir(dirPath, { withFileTypes: true })) {
        const entryPath = path.join(dirPath, entry.name);
        if (shouldIgnorePath(entryPath)) continue;
        if (entry.isDirectory()) await walk(entryPath, col);
        else if (entry.isFile() && isSupportedFile(entryPath)) await indexFile(entryPath, col);
    }
}

/** Обходит INDEX_ROOT и индексирует все подходящие файлы. */
export async function indexProject(col) {
    totalIndexedFiles = 0;
    try { await fs.promises.access(INDEX_ROOT); } catch { return; }
    await walk(INDEX_ROOT, col);
}
 ...
 */
export async function buildIndex(col, forceRebuild = false) {
    if (forceRebuild) {
        try { col.deleteByFilterSync("1=1"); } catch (err) {
            console.error(`${LOG_PREFIX} Could not clear index: ${err.message}`);
        }
    }
    console.error(`${LOG_PREFIX} Indexing started (root=${INDEX_ROOT})...`);
    await indexProject(col);
    console.error(`${LOG_PREFIX} Indexing done. Files: ${totalIndexedFiles}`);
    return { ok: true, projectRoot: PROJECT_ROOT, filesIndexed: totalIndexedFiles };
}